...
def load_dataset_from_directory(directory, sample_size=None):
    ...
    # 1. Extract IDs from filenames (e.g., "26396_leu..." -> 26396)
    file_map = {} # id -> file_path
    for f in files:
        try:
            # Assumes split_cluster.py naming format: "{id}_{original_name}"
            coin_id = int(f.name.split('_')[0])
            file_map[coin_id] = str(f.absolute())
        except ValueError:
            print(f"  ⚠️ Could not parse ID from: {f.name}")
            continue
    ...
    for i in range(0, len(all_ids), batch_size):
        ...
        for row in rows:
            # Merge DB data with the local file path
...
def save_results(records, cluster_labels, confidences, embedding_2d, summary, output_dir, features=None):
    ...
    # ADD THIS: Save high-dimensional features for re-clustering later
...
def drill_down_cluster(target_cluster_id, features, df, output_dir):
    ...
    # Convert dataframe subset back to records list format for compatibility
    sub_records = df.loc[cluster_mask].to_dict('records')
    
    # Fix keys if CSV headers differ from internal record keys
    for r in sub_records:
        if 'parser_period' in r: r['period'] = r.pop('parser_period')
    
    print(f"   Analyzing {len(sub_features)} coins in sub-space...")

    # 2. Re-run UMAP just for this subset
    # We use fewer neighbors to focus on local variations rather than global structure
    import umap
    print("   Re-calculating UMAP manifold...")
    reducer = umap.UMAP(
        n_components=5,       # Keep dims low for HDBSCAN
        n_neighbors=10,       # Lower neighbors (default is usually 15-30)
        min_dist=0.0,         # Allow points to clump tightly
        metric='cosine',
        random_state=42
    )
    sub_embedding = reducer.fit_transform(sub_features)

    # 3. Re-run HDBSCAN with sensitive settings
    import hdbscan
    print("   Re-clustering...")
    clusterer = hdbscan.HDBSCAN(
        min_cluster_size=5,   # Much smaller minimum size (detect small groups)
        min_samples=2,        # Allow tight, small groups
        cluster_selection_method='leaf', # 'leaf' splits clusters more aggressively
    )
    sub_labels = clusterer.fit_predict(sub_embedding)
    
    # 4. Generate Sub-Report in a new folder
    sub_output_dir = Path(output_dir) / f"subcluster_{target_cluster_id}"
    sub_output_dir.mkdir(exist_ok=True)
    
    # Get 2D for visualization
    viz_reducer = umap.UMAP(n_components=2, n_neighbors=10, min_dist=0.1)
    sub_embedding_2d = viz_reducer.fit_transform(sub_features)
    
    # Fake confidences (or calculate real ones via membership_vector)
    sub_confidences = np.ones(len(sub_labels))
    
    _, sub_summary = analyze_clusters(sub_records, sub_labels, sub_confidences)
    
    # Save results (recursive)
...
def main():
    ...
    # Save results
    df = save_results(valid_records, cluster_labels, confidences, embedding_2d, summary, args.output, features=features)
