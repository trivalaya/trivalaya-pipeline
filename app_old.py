...
os.environ["NNPACK_MODE"] = "0"          # Disables the slow hardware check
os.environ["OMP_NUM_THREADS"] = "1"      # Prevents thread fighting on 2-vCPU node
...
def extract_coin_from_image(image_bgr: np.ndarray) -> tuple:
    ...
    INCLUDES: Resize protection, Dominant Object check, and Center-Weighted scoring.
    """
    if image_bgr is None:
        return None, {"error": "invalid_image"}
    
    # 1. Resize massive images to prevent hang (Processing Scale)
    ...
    if max(h_raw, w_raw) > max_dim:
        ...
        processing_image = cv2.resize(image_bgr, (new_w, new_h), interpolation=cv2.INTER_AREA)
    else:
        processing_image = image_bgr.copy()
    
    # 2. Pre-processing
    h, w = processing_image.shape[:2]
    total_area = h * w
    center_x, center_y = w / 2, h / 2 # Cache center for distance calc
    ...
    # --- DEFINE EDGE ZONE (Fixing your NameError) ---
    v = np.median(gray)
    sigma = Layer1Config.CANNY_SIGMA
    lower = int(max(0, (1.0 - sigma) * v))
    upper = int(min(255, (1.0 + sigma) * v))
    edges = cv2.Canny(gray, lower, upper)
    # Dilate edges to create a "landing zone" for validation
    ...
    for c in contours:
        ...
        # Filter tiny noise
        # Note: We use a strict 1% filter to stop it picking up dust/pebbles
        if area_ratio < 0.01: continue 
        
        # Large Coin Check
        is_dominant = area_ratio > 0.40
        if area_ratio > 0.98: continue 
        
        # --- SCORING FACTORS ---
        
        # A. Center Distance (New)
        ...
        if not is_dominant:
            ...
            p_pixels = cv2.countNonZero(perimeter_mask)
            if p_pixels > 0:
                overlap = cv2.bitwise_and(perimeter_mask, edge_zone)
                edge_support = cv2.countNonZero(overlap) / p_pixels
            else:
                edge_support = 0.0
        ...
        # Boost based on Size (The single strongest predictor)
        # Big things are more likely to be the subject than small things
        size_boost = 1.0 + (area_ratio * 3.0) 
        
        final_score = coin_likelihood * size_boost
    ...
    # 5. Crop Strategy
    if best["is_dominant"]:
        # Strategy A: Use Full Image (Dominated)
        cropped = image_bgr.copy() 
        info_status = "success_dominant_object"
    else:
        # Strategy B: Crop
        x, y, bw, bh = best["bbox"]
        if scale != 1.0:
            ...
            bw = int(bw * rescale_factor)
            bh = int(bh * rescale_factor)
            w_orig, h_orig = w_raw, h_raw
        else:
            w_orig, h_orig = w, h
        ...
        info_status = "success_crop"

    # 6. Pad to Square (White Background)
    h_c, w_c = cropped.shape[:2]
    dim = max(h_c, w_c)
    
    square_img = np.ones((dim, dim, 3), dtype=np.uint8) * 255
    y_off = (dim - h_c) // 2
    x_off = (dim - w_c) // 2
    
    if h_c > 0 and w_c > 0:
        square_img[y_off:y_off+h_c, x_off:x_off+w_c] = cropped
